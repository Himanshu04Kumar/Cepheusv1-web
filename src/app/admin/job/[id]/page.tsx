  ...
  const runAction = async (action, data) => {
    ...
    try {
      const res = await fetch('/api/admin/update', {
        ...
        body: JSON.stringify({ action, bookingId: id, data, adminRole: profile?.role }),
      });
    ...
    <div className="min-h-screen bg-slate-950 text-white p-4 md:p-8 transition-colors">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex justify-between items-end border-b border-slate-900 pb-8 text-white">
          <div><p className="text-[10px] font-black text-blue-500 uppercase tracking-[0.2em] mb-1">Command Unit</p><h1 className="text-4xl font-black uppercase tracking-tighter">{booking?.customer_name}</h1></div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          <div className="space-y-6">
            ...
            <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800 shadow-xl space-y-4 text-white text-white text-white">
            ...
            <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800 shadow-xl space-y-4 text-white">
              ...
              <select className="w-full bg-slate-950 p-3 rounded-xl border border-slate-800 text-[10px] font-bold text-slate-300 outline-none" value={noteStage} onChange={e => setNoteStage(e.target.value)}>{stages.map(s => <option key={s} value={s}>{s.replace(/_/g, ' ')}</option>)}</select>
            ...
            <div className="bg-slate-900 p-8 rounded-[2rem] border border-slate-800 shadow-2xl space-y-6 text-white text-white">
              ...
              {selectedOption ? (
                <div className="bg-green-600/10 border border-green-500/30 p-6 rounded-3xl flex justify-between items-center text-white">
                  <div><p className="text-[10px] font-black text-green-500 uppercase tracking-widest mb-1 text-white text-white">Verified Selection</p><p className="text-lg font-black uppercase text-white">{selectedOption.option_name}</p></div>
                ...
                <div className="space-y-4 text-white text-white">
                  {options.map((opt, i) => (
                    <div key={i} className={`grid grid-cols-12 gap-1 p-4 rounded-2xl border bg-slate-950 border-slate-800 text-white`}>
                      <div className="col-span-4 border-r border-white/10 pr-2"><input placeholder="Option" className="w-full bg-transparent text-xs font-bold uppercase outline-none text-white" value={opt.option_name} onChange={e => {const n=[...options]; n[i].option_name=e.target.value; setOptions(n);}}/></div>
                      <div className="col-span-5 border-r border-white/10 px-2"><input placeholder="Details..." className="w-full bg-transparent text-[10px] outline-none text-slate-400" value={opt.description} onChange={e => {const n=[...options]; n[i].description=e.target.value; setOptions(n);}}/></div>
                      <div className="col-span-2 px-2 text-white"><input placeholder="₹" className="w-full bg-transparent text-xs font-black text-amber-500 outline-none" type="number" value={opt.price} onChange={e => {const n=[...options]; n[i].price=e.target.value; setOptions(n);}}/></div>
                      <div className="col-span-1 flex justify-end text-white"><button onClick={() => setOptions(options.filter((_, idx) => idx !== i))} className="text-slate-700 hover:text-red-500 transition-colors"><Trash2 size={14}/></button></div>
                    </div>
                  ))}
          ...
          <div className="space-y-6 text-white text-white">
            <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800 shadow-xl space-y-6 h-full max-h-[800px] overflow-y-auto scrollbar-none text-white text-white">
              ...
              <div className="space-y-3 text-white text-white">{existingPhotos.map((p, i) => (<div key={i} className="group relative aspect-video rounded-xl overflow-hidden border border-slate-800 bg-slate-950"><img src={p.removed_part_photo} className="w-full h-full object-cover opacity-60 group-hover:opacity-100 transition-all"/><p className="absolute bottom-2 left-2 text-[7px] font-black uppercase bg-black/60 px-1.5 py-0.5 rounded text-white">{p.removed_part_name}</p></div>))}</div>
