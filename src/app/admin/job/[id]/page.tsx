    ...
    <div className="min-h-screen bg-slate-950 text-white p-4 md:p-8 transition-colors">
      <div className="max-w-7xl mx-auto space-y-8 text-white">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="space-y-6">
            <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800 shadow-xl space-y-4">
              ...
              <div className="grid grid-cols-1 gap-2">
                {['PICKED_UP', 'DIAGNOSING', 'QUALITY_CHECK'].map(s => {
                  ...
                  const isStaffLocked = isReversal && profile?.role !== 'SUPER_ADMIN';
                  ...
                  return (
                    <button
                      ...
                      className={`p-3 text-[10px] font-black rounded-xl border transition-all uppercase tracking-widest relative group flex items-center justify-center gap-2 ${
                        ...
                        isStaffLocked ? 'border-slate-800 text-slate-700 cursor-not-allowed bg-slate-950' :
                        'border-slate-800 hover:bg-slate-800 text-slate-500'
                      }`}
                    >
                      {isStaffLocked && <Lock size={10} className="text-red-500" />}
                      {s.replace(/_/g, ' ')}
                    </button>
                  );
                })}
            ...
            <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800 shadow-xl space-y-4 text-white">
              ...
              <div className="space-y-3">
                 <input placeholder="Estimated Window" className="w-full bg-slate-950 p-3 rounded-xl border border-slate-800 text-[10px] outline-none text-white" value={deliveryWindow} onChange={e => setDeliveryWindow(e.target.value)} />
            ...
            <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800 shadow-xl space-y-4 text-white">
              ...
              <textarea placeholder="Finding..." className="w-full bg-slate-950 p-4 rounded-xl border border-slate-800 text-[10px] h-20 outline-none text-white" value={techNote} onChange={e => setTechNote(e.target.value)} />
              <button disabled={!techNote} onClick={() => runAction('ADD_COMMENT', { stage: noteStage, text: techNote })} className="w-full bg-blue-600 text-white py-3 rounded-xl font-black uppercase text-[10px] disabled:opacity-30">Push Note</button>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 space-y-8">
            <div className="bg-slate-900 p-8 rounded-[2rem] border border-slate-800 shadow-2xl space-y-6 text-white text-white">
              <div className="flex justify-between items-center">
                ...
                {profile?.role === 'SUPER_ADMIN' && <button onClick={() => setOptions([...options, { option_name: '', description: '', price: '' }])} className="p-2 bg-amber-500/10 rounded-full text-amber-500 hover:bg-amber-500 hover:text-white transition-all"><Plus size={16}/></button>}
              </div>
              {selectedOption ? (
                ...
                <div className="space-y-4 text-white">
                  {options.map((opt, i) => (
                    <div key={i} className={`grid grid-cols-12 gap-3 p-4 rounded-2xl border bg-slate-950 border-slate-800`}>
                      <div className="col-span-4"><input className="w-full bg-transparent text-xs font-bold uppercase outline-none text-white" value={opt.option_name} onChange={e => {const n=[...options]; n[i].option_name=e.target.value; setOptions(n);}}/></div>
                      <div className="col-span-5"><input className="w-full bg-transparent text-[10px] outline-none text-slate-400" value={opt.description} onChange={e => {const n=[...options]; n[i].description=e.target.value; setOptions(n);}}/></div>
                      <div className="col-span-2 text-white"><input className="w-full bg-transparent text-xs font-black text-amber-500 outline-none" type="number" value={opt.price} onChange={e => {const n=[...options]; n[i].price=e.target.value; setOptions(n);}}/></div>
                  ...
                  <button onClick={() => runAction('PUBLISH_OPTIONS', { options })} className="w-full bg-amber-600 text-white py-4 rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-amber-500">Publish to Customer</button>
                </div>
              )}
            </div>
            ...
            <div className="bg-slate-900 p-8 rounded-[2rem] border border-slate-800 shadow-2xl space-y-6">
              ...
              <label className="flex flex-col items-center justify-center aspect-video border-2 border-dashed border-slate-800 rounded-3xl cursor-pointer hover:bg-slate-800/50 transition-all overflow-hidden relative">
                {partDoc.photo ? <img src={partDoc.photo} className="w-full h-full object-cover"/> : <div className="text-center space-y-2"><UploadCloud size={32} className="text-slate-700 mx-auto"/><p className="text-[10px] font-black uppercase text-slate-600">Snap Photo</p></div>}
                <input type="file" className="hidden" onChange={e => handleUpload(e.target.files[0])}/>
              </label>
              <div className="grid grid-cols-2 gap-4">
                <input placeholder="Label" className="bg-slate-950 p-4 rounded-xl border border-slate-800 text-xs outline-none focus:ring-1 ring-purple-500 text-white" value={partDoc.name} onChange={e => setPartDoc({...partDoc, name: e.target.value})}/>
                <input placeholder="S/N" className="bg-slate-950 p-4 rounded-xl border border-slate-800 text-xs outline-none focus:ring-1 ring-purple-500 text-white" value={partDoc.serial} onChange={e => setPartDoc({...partDoc, serial: e.target.value})}/>
              </div>
          ...
          {/* HISTORICAL LOG */}
          <div className="space-y-6">
            <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800 shadow-xl space-y-6 h-full max-h-[800px] overflow-y-auto scrollbar-none">
              <h2 className="text-[10px] font-black uppercase tracking-widest text-blue-500 flex items-center gap-2 sticky top-0 bg-slate-900 pb-2 z-10"><History size={14}/> Records</h2>
              <div className="space-y-3">
              ...
              <div className="space-y-3 border-t border-slate-800 pt-6">
                 {existingComments.map((c, i) => (
                   <div key={i} className="bg-slate-950 p-3 rounded-xl border border-slate-800">
                      <p className="text-[7px] font-black text-blue-500 uppercase mb-1">{c.stage.replace(/_/g, ' ')}</p>
                      <p className="text-[10px] text-slate-400 font-medium">"{c.comment_text}"</p>
                   </div>
                 ))}
              </div>
            </div>
        ...
        {statusMsg && <div className="fixed bottom-8 right-8 p-4 bg-blue-600 text-white rounded-2xl font-bold shadow-2xl z-50 uppercase text-[10px] tracking-widest animate-pulse">{statusMsg}</div>}
      </div>
    </div>
  );
}
