        ...
        <div className="grid lg:grid-cols-3 gap-8">
          <div className="space-y-6">
            ...
            <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800 shadow-xl space-y-4">
              ...
              <textarea placeholder="Add a stage comment (e.g. 'Broken port')" className="w-full bg-slate-950 p-4 rounded-xl border border-slate-800 text-[10px] h-24 outline-none text-white" value={techNote} onChange={e => setTechNote(e.target.value)} />
              <button disabled={!techNote} onClick={() => runAction('ADD_COMMENT', { stage: booking?.status, text: techNote })} className="w-full bg-blue-600 text-white py-3 rounded-xl font-black uppercase text-[10px] tracking-widest disabled:opacity-30">Push Note to Timeline</button>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 space-y-8">
            <div className="bg-slate-900 p-8 rounded-[2rem] border border-slate-800 shadow-2xl space-y-6 text-white">
              ...
              {selectedOption ? (
                <div className="bg-green-600/10 border border-green-500/30 p-6 rounded-3xl flex justify-between items-center text-white">
                  <div><p className="text-[10px] font-black text-green-500 uppercase tracking-widest mb-1">Selection Verified</p><p className="text-lg font-black uppercase text-white">{selectedOption.option_name}</p></div>
                ...
                <div className="space-y-4 text-white">
                  {options.map((opt, i) => (
                    <div key={i} className={`grid grid-cols-12 gap-3 p-4 rounded-2xl border ${i === 0 ? 'bg-slate-900 border-blue-500/30' : 'bg-slate-950 border-slate-800'}`}>
                      <div className="col-span-4"><input readOnly={i===0} placeholder="Option" className="w-full bg-transparent text-xs font-bold uppercase outline-none text-white" value={opt.option_name} onChange={e => {const n=[...options]; n[i].option_name=e.target.value; setOptions(n);}}/></div>
                      <div className="col-span-5"><input readOnly={i===0} placeholder="Desc..." className="w-full bg-transparent text-[10px] outline-none text-slate-400" value={opt.description} onChange={e => {const n=[...options]; n[i].description=e.target.value; setOptions(n);}}/></div>
                      <div className="col-span-2 text-white"><input readOnly={i===0} placeholder="₹" className="w-full bg-transparent text-xs font-black text-amber-500 outline-none" type="number" value={opt.price} onChange={e => {const n=[...options]; n[i].price=e.target.value; setOptions(n);}}/></div>
                  ...
                  <button onClick={() => runAction('PUBLISH_OPTIONS', { options })} className="w-full bg-amber-600 text-white py-4 rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-amber-500 transition-all">Publish to Customer</button>
                </div>
              )}
            </div>
            ...
            <div className="bg-slate-900 p-8 rounded-[2rem] border border-slate-800 shadow-2xl space-y-6">
              ...
              <button onClick={() => runAction('DOCUMENT_PART', partDoc)} className="w-full bg-purple-600 text-white py-4 rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-purple-500">Push to Evidence Log</button>
            </div>
          </div>
        </div>
