...
export default function AdvancedAdminManagement() {
  ...
  // Event 4: Options State (with mandatory first option)
    ...
    <div className="min-h-screen bg-slate-950 text-white p-4 md:p-8 transition-colors">
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
           ...
           <div className="bg-slate-900 p-5 rounded-[1.5rem] border border-slate-800/50 shadow-xl">
             <p className="text-[10px] text-slate-500 uppercase font-black mb-1 tracking-widest">Logged On</p>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          <div className="space-y-6">
            ...
            <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800 shadow-xl space-y-4">
              <h2 className="text-xs font-black uppercase tracking-widest text-orange-500 flex items-center gap-2"><Truck size={14}/> Dispatch & Logistics</h2>
              <div className="space-y-3">
                 <input
                   placeholder="Estimated Delivery (e.g. Today, 6 PM)"
                   className="w-full bg-slate-950 p-3 rounded-xl border border-slate-800 text-[10px] outline-none"
                   value={deliveryWindow}
                   onChange={e => setDeliveryWindow(e.target.value)}
                 />
                 <button
                   onClick={() => runAction('UPDATE_STATUS', { status: 'OUT_FOR_DELIVERY', deliveryWindow })}
                   className={`w-full p-3 text-[10px] font-black rounded-xl border transition-all uppercase tracking-widest ${booking?.status === 'OUT_FOR_DELIVERY' ? 'bg-orange-600 border-orange-500' : 'border-slate-800 hover:bg-orange-900/20 text-orange-500'}`}
                 >
                   Out For Delivery
                 </button>
                 <button
                   onClick={() => runAction('UPDATE_STATUS', { status: 'DELIVERED' })}
                   className={`w-full p-3 text-[10px] font-black rounded-xl border transition-all uppercase tracking-widest ${booking?.status === 'DELIVERED' ? 'bg-green-600 border-green-500' : 'border-slate-800 hover:bg-green-900/20 text-green-500'}`}
                 >
                   Mark Delivered
                 </button>
              </div>
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-slate-900 p-8 rounded-[2rem] border border-slate-800 shadow-2xl space-y-6">
              ...
              {selectedOption ? (
                <div className="bg-green-600/10 border border-green-500/30 p-6 rounded-3xl flex justify-between items-center">
                  <div>
                    <p className="text-[10px] font-black text-green-500 uppercase tracking-widest mb-1">Selection Verified</p>
                    <p className="text-lg font-black uppercase">{selectedOption.option_name}</p>
                  ...
                  <div className="text-right">
                     ...
                     <div className="flex items-center gap-1 text-[10px] font-black text-green-500 uppercase mt-1"><CheckCircle2 size={12}/> Paid</div>
                  </div>
                </div>
              ) : (
                <div className="space-y-4">
                  {options.map((opt, i) => (
                    <div key={i} className={`grid grid-cols-12 gap-3 p-4 rounded-2xl border ${i === 0 ? 'bg-slate-900 border-blue-500/30' : 'bg-slate-950 border-slate-800'}`}>
                      <div className="col-span-4"><input readOnly={i===0} placeholder="Option Name" className="w-full bg-transparent text-xs font-bold uppercase outline-none" value={opt.option_name} onChange={e => {const n=[...options]; n[i].option_name=e.target.value; setOptions(n);}}/></div>
                      <div className="col-span-5"><input readOnly={i===0} placeholder="Description..." className="w-full bg-transparent text-[10px] outline-none text-slate-400" value={opt.description} onChange={e => {const n=[...options]; n[i].description=e.target.value; setOptions(n);}}/></div>
                      <div className="col-span-2"><input readOnly={i===0} placeholder="Price" className="w-full bg-transparent text-xs font-black text-amber-500 outline-none" type="number" value={opt.price} onChange={e => {const n=[...options]; n[i].price=e.target.value; setOptions(n);}}/></div>
            ...
            {/* EVENT 5: EVIDENCE LOG (SINGLE PHOTO) */}
            <div className="bg-slate-900 p-8 rounded-[2rem] border border-slate-800 shadow-2xl space-y-6">
                   ...
                   <label className="flex flex-col items-center justify-center aspect-video border-2 border-dashed border-slate-800 rounded-3xl cursor-pointer hover:bg-slate-800/50 transition-all overflow-hidden relative">
                      {partDoc.photo ? <img src={partDoc.photo} className="w-full h-full object-cover"/> : (
                        <div className="text-center space-y-2">
                           <UploadCloud size={32} className="text-slate-700 mx-auto"/>
                           <p className="text-[10px] font-black uppercase text-slate-600 tracking-widest">Click to upload repair proof</p>
                        </div>
                      )}
                      <input type="file" className="hidden" onChange={e => handleUpload(e.target.files[0])}/>
                   </label>
                   <div className="grid grid-cols-2 gap-4">
                      <input placeholder="Part Name / Stage Name" className="bg-slate-950 p-4 rounded-xl border border-slate-800 text-xs outline-none focus:ring-1 ring-purple-500" value={partDoc.name} onChange={e => setPartDoc({...partDoc, name: e.target.value})}/>
                      <input placeholder="Serial / Reference Number" className="bg-slate-950 p-4 rounded-xl border border-slate-800 text-xs outline-none focus:ring-1 ring-purple-500" value={partDoc.serial} onChange={e => setPartDoc({...partDoc, serial: e.target.value})}/>
                   </div>
