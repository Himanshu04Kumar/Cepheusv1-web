...
export default function AdvancedAdminManagement() {
  ...
  const [profile, setProfile] = useState(null); // Critical for role-based locking
  ...
  useEffect(() => {
    async function fetchAllData() {
      // 1. Get User Session & Role (Essential for Security Enforcment)
    ...
    try {
      const res = await fetch('/api/admin/update', {
        ...
        body: JSON.stringify({
          ...
          bookingId: id,
          data,
          adminRole: profile?.role // PASSING ROLE TO BACKEND
        }),
      });
      const result = await res.json();

      if (!res.ok) {
        // If the backend rejects (e.g. reversal attempt), show the error clearly
        throw new Error(result.error || 'API Rejection');
      }
      ...
      // Show alert if it's a security lock issue
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-4 md:p-8 transition-colors">
      <div className="max-w-7xl mx-auto space-y-8 text-white text-white">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 text-white">
          ...
          <div className="space-y-6">
            <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800 shadow-xl space-y-4">
              <h2 className="text-xs font-black uppercase tracking-widest text-slate-500 flex items-center gap-2 text-white"><RefreshCw size={14}/> Set Stage</h2>
              <div className="grid grid-cols-1 gap-2 text-white text-white">
                {['PICKED_UP', 'DIAGNOSING', 'QUALITY_CHECK'].map(s => {
                  ...
                  const isReversal = targetIndex < currentIndex;
                  // If it's a reversal and NOT super admin, disable the button locally too
                  const isLocked = isReversal && profile?.role !== 'SUPER_ADMIN';

                  return (
                    <button
                      key={s}
                      disabled={isLocked}
                      onClick={() => runAction('UPDATE_STATUS', { status: s })}
                      className={`p-3 text-[10px] font-black rounded-xl border transition-all uppercase tracking-widest relative group flex items-center justify-center gap-2 ${
                        booking?.status === s ? 'bg-blue-600 border-blue-500 text-white' :
                        isLocked ? 'border-slate-800 text-slate-700 cursor-not-allowed bg-slate-950' :
              ...
              {profile?.role !== 'SUPER_ADMIN' && <p className="text-[8px] text-slate-600 italic text-center uppercase tracking-widest">Employee Lock: Stages are irreversible.</p>}
            </div>
            ...
            <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800 shadow-xl space-y-4 text-white">
              ...
              <div className="space-y-3 text-white text-white">
                 <input placeholder="Estimated Window" className="w-full bg-slate-950 p-3 rounded-xl border border-slate-800 text-[10px] outline-none text-white text-white" value={deliveryWindow} onChange={e => setDeliveryWindow(e.target.value)} />
                 <button onClick={() => runAction('UPDATE_STATUS', { status: 'OUT_FOR_DELIVERY', deliveryWindow })} className="w-full p-3 text-[10px] font-black rounded-xl border border-slate-800 hover:bg-orange-900/20 text-orange-500 uppercase text-white">Out For Delivery</button>
                 <button onClick={() => runAction('UPDATE_STATUS', { status: 'DELIVERED' })} className="w-full p-3 text-[10px] font-black rounded-xl border border-slate-800 hover:bg-green-900/20 text-green-500 uppercase text-white">Mark Delivered</button>
              </div>
            </div>
            ...
            <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800 shadow-xl space-y-4 text-white">
              ...
              <select className="w-full bg-slate-950 p-3 rounded-xl border border-slate-800 text-[10px] font-bold text-slate-300 outline-none text-white" value={noteStage} onChange={e => setNoteStage(e.target.value)}>
              ...
              <button disabled={!techNote} onClick={() => runAction('ADD_COMMENT', { stage: noteStage, text: techNote })} className="w-full bg-blue-600 text-white py-3 rounded-xl font-black uppercase text-[10px] disabled:opacity-30 text-white">Push Note</button>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 space-y-8 text-white">
            <div className="bg-slate-900 p-8 rounded-[2rem] border border-slate-800 shadow-2xl space-y-6 text-white text-white">
              <div className="flex justify-between items-center text-white">
                ...
                {profile?.role === 'SUPER_ADMIN' && <button onClick={() => setOptions([...options, { option_name: '', description: '', price: '' }])} className="p-2 bg-amber-500/10 rounded-full text-amber-500 hover:bg-amber-500 hover:text-white transition-all text-white"><Plus size={16}/></button>}
              </div>
              {selectedOption ? (
                <div className="bg-green-600/10 border border-green-500/30 p-6 rounded-3xl flex justify-between items-center text-white text-white">
                  <div><p className="text-[10px] font-black text-green-500 uppercase tracking-widest mb-1 text-white">Paid & Verified</p><p className="text-lg font-black uppercase text-white">{selectedOption.option_name}</p></div>
                  <div className="text-right text-white"><p className="text-2xl font-black text-green-400 font-mono text-white">₹{selectedOption.price}</p><div className="flex items-center gap-1 text-[10px] font-black text-green-500 uppercase mt-1 text-white"><CheckCircle2 size={12}/> Success</div></div>
                </div>
              ) : (
                <div className="space-y-4 text-white text-white">
                  {options.map((opt, i) => (
                    <div key={i} className={`grid grid-cols-12 gap-3 p-4 rounded-2xl border bg-slate-950 border-slate-800 text-white`}>
                      <div className="col-span-4 text-white"><input className="w-full bg-transparent text-xs font-bold uppercase outline-none text-white" value={opt.option_name} onChange={e => {const n=[...options]; n[i].option_name=e.target.value; setOptions(n);}}/></div>
                      <div className="col-span-5 text-white"><input className="w-full bg-transparent text-[10px] outline-none text-slate-400 text-white" value={opt.description} onChange={e => {const n=[...options]; n[i].description=e.target.value; setOptions(n);}}/></div>
                      <div className="col-span-2 text-white"><input className="w-full bg-transparent text-xs font-black text-amber-500 outline-none text-white" type="number" value={opt.price} onChange={e => {const n=[...options]; n[i].price=e.target.value; setOptions(n);}}/></div>
                  ...
                  <button onClick={() => runAction('PUBLISH_OPTIONS', { options })} className="w-full bg-amber-600 text-white py-4 rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-amber-500 text-white">Publish Options</button>
                </div>
              )}
            </div>
            ...
            <div className="bg-slate-900 p-8 rounded-[2rem] border border-slate-800 shadow-2xl space-y-6 text-white text-white">
              ...
              <label className="flex flex-col items-center justify-center aspect-video border-2 border-dashed border-slate-800 rounded-3xl cursor-pointer hover:bg-slate-800/50 transition-all overflow-hidden relative text-white">
                {partDoc.photo ? <img src={partDoc.photo} className="w-full h-full object-cover"/> : <div className="text-center space-y-2 text-white"><UploadCloud size={32} className="text-slate-700 mx-auto text-white"/><p className="text-[10px] font-black uppercase text-slate-600 text-white">Snap Photo</p></div>}
            ...
            <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800 shadow-xl space-y-6 h-full max-h-[800px] overflow-y-auto scrollbar-none text-white text-white">
              ...
              <div className="space-y-3 text-white text-white">
                 {existingPhotos.map((p, i) => (
                   <div key={i} className="group relative aspect-video rounded-xl overflow-hidden border border-slate-800 bg-slate-950 text-white">
                      <img src={p.removed_part_photo} className="w-full h-full object-cover opacity-60 group-hover:opacity-100 transition-all text-white"/>
                      <p className="absolute bottom-2 left-2 text-[7px] font-black uppercase bg-black/60 px-1.5 py-0.5 rounded text-white text-white">{p.removed_part_name}</p>
                   </div>
                 ))}
              </div>
              <div className="space-y-3 border-t border-slate-800 pt-6 text-white text-white">
                 {existingComments.map((c, i) => (
                   <div key={i} className="bg-slate-950 p-3 rounded-xl border border-slate-800 text-white">
                      <p className="text-[7px] font-black text-blue-500 uppercase mb-1 text-white">{c.stage.replace(/_/g, ' ')}</p>
                      <p className="text-[10px] text-slate-400 font-medium text-white italic">"{c.comment_text}"</p>
                   </div>
                 ))}
              </div>
            </div>
