    ...
    try {
      const res = await fetch('/api/admin/update', {
        ...
        body: JSON.stringify({
          ...
          adminEmail: profile?.email // REQUIRED FOR AUDIT LOG POPULATION
        }),
      });
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-4 md:p-8 transition-colors">
      <div className="max-w-7xl mx-auto space-y-8">
        <Link href="/admin" className="text-slate-500 hover:text-white flex items-center gap-2 mb-4 text-white text-white text-white"><ArrowLeft size={16} /> Dashboard</Link>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-white">
           ...
           <div className="bg-slate-900 p-5 rounded-[1.5rem] border border-slate-800/50 shadow-xl text-white text-white text-white"><p className="text-[10px] text-slate-500 uppercase font-black mb-1">Registry ID</p><p className="text-xs font-mono font-bold text-blue-400 uppercase">{id.slice(0,8)}</p></div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 text-white">
          ...
          <div className="space-y-6 text-white text-white text-white">
            ...
            <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800 shadow-xl space-y-4 text-white">
              ...
              <select className="w-full bg-slate-950 p-3 rounded-xl border border-slate-800 text-[10px] font-bold text-slate-300 outline-none text-white text-white text-white" value={noteStage} onChange={e => setNoteStage(e.target.value)}>{stages.map(s => <option key={s} value={s}>{s.replace(/_/g, ' ')}</option>)}</select>
              <textarea placeholder="Finding..." className="w-full bg-slate-950 p-4 rounded-xl border border-slate-800 text-[10px] h-20 outline-none text-white text-white text-white" value={techNote} onChange={e => setTechNote(e.target.value)} />
          ...
          <div className="lg:col-span-2 space-y-8 text-white">
            <div className="bg-slate-900 p-8 rounded-[2rem] border border-slate-800 shadow-2xl space-y-6 text-white text-white text-white">
              <div className="flex justify-between items-center text-white">
                ...
                <button onClick={addOption} className="p-2 bg-amber-500/10 rounded-full text-amber-500 hover:bg-amber-500 hover:text-white transition-all text-white text-white text-white"><Plus size={16}/></button>
              </div>
              {selectedOption ? (
                <div className="bg-green-600/10 border border-green-500/30 p-6 rounded-3xl flex justify-between items-center text-white text-white text-white">
                  <div><p className="text-[10px] font-black text-green-500 uppercase tracking-widest mb-1 text-white text-white text-white">Verified Selection</p><p className="text-lg font-black uppercase text-white">{selectedOption.option_name}</p></div>
                  <div className="text-right text-white"><p className="text-2xl font-black text-green-400 font-mono">₹{selectedOption.price}</p><div className="flex items-center gap-1 text-[10px] font-black text-green-500 uppercase mt-1 text-white text-white"><CheckCircle2 size={12}/> Success</div></div>
                </div>
              ) : (
                <div className="space-y-4 text-white text-white text-white">
                  {options.map((opt, i) => (
                    <div key={i} className={`grid grid-cols-12 gap-1 p-4 rounded-2xl border bg-slate-950 border-slate-800 text-white`}>
                      <div className="col-span-4 border-r border-white/10 pr-2 text-white text-white"><input placeholder="Option" className="w-full bg-transparent text-xs font-bold uppercase outline-none text-white text-white" value={opt.option_name} onChange={e => updateOption(i, 'option_name', e.target.value)}/></div>
                      <div className="col-span-5 border-r border-white/10 px-2 text-white text-white"><input placeholder="Details..." className="w-full bg-transparent text-[10px] outline-none text-slate-400" value={opt.description} onChange={e => updateOption(i, 'description', e.target.value)}/></div>
                      <div className="col-span-2 px-2 text-white text-white text-white"><input placeholder="₹" className="w-full bg-transparent text-xs font-black text-amber-500 outline-none" type="number" value={opt.price} onChange={e => updateOption(i, 'price', e.target.value)}/></div>
                  ...
                  <button onClick={addOption} className="w-full py-3 border-2 border-dashed border-slate-800 rounded-2xl text-[10px] font-black uppercase text-slate-600 hover:text-blue-500 hover:border-blue-500/50 transition-all flex items-center justify-center gap-2 text-white text-white text-white text-white text-white">
                       <Plus size={14}/> Add Another Path
                  </button>
                  <button onClick={() => runAction('PUBLISH_OPTIONS', { options })} className="w-full bg-amber-600 text-white py-4 rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-amber-500 text-white text-white">Publish Options</button>
                </div>
              )}
            </div>

            <div className="bg-slate-900 p-8 rounded-[2rem] border border-slate-800 shadow-2xl space-y-6 text-white text-white text-white text-white text-white">
              ...
              <label className="flex flex-col items-center justify-center aspect-video border-2 border-dashed border-slate-800 rounded-3xl cursor-pointer hover:bg-slate-800/50 transition-all overflow-hidden relative text-white text-white">
                {partDoc.photo ? <img src={partDoc.photo} className="w-full h-full object-cover"/> : <div className="text-center space-y-2 text-white"><UploadCloud size={32} className="text-slate-700 mx-auto text-white text-white"/><p className="text-[10px] font-black uppercase text-slate-600">Snap Photo</p></div>}
                <input type="file" className="hidden text-white text-white" onChange={e => handleUpload(e.target.files[0])}/>
              </label>
              ...
              <button onClick={() => runAction('DOCUMENT_PART', partDoc)} className="w-full bg-purple-600 text-white py-4 rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-purple-500 text-white text-white">Push to Log</button>
            </div>
          </div>

          <div className="space-y-6 text-white text-white text-white">
            <div className="bg-slate-900 p-6 rounded-3xl border border-slate-800 shadow-xl space-y-6 h-full max-h-[800px] overflow-y-auto scrollbar-none text-white text-white text-white text-white">
              <h2 className="text-[10px] font-black uppercase tracking-widest text-blue-500 flex items-center gap-2 sticky top-0 bg-slate-900 pb-2 z-10 text-white text-white text-white text-white text-white"><History size={14}/> Records</h2>
              <div className="space-y-3 text-white text-white text-white text-white">{existingPhotos.map((p, i) => (<div key={i} className="group relative aspect-video rounded-xl overflow-hidden border border-slate-800 bg-slate-950 text-white"><img src={p.removed_part_photo} className="w-full h-full object-cover opacity-60 group-hover:opacity-100 transition-all"/><p className="absolute bottom-2 left-2 text-[7px] font-black uppercase bg-black/60 px-1.5 py-0.5 rounded text-white text-white">{p.removed_part_name}</p></div>))}</div>
              <div className="space-y-3 border-t border-slate-800 pt-6 text-white text-white text-white text-white text-white">{existingComments.map((c, i) => (<div key={i} className="bg-slate-950 p-3 rounded-xl border border-slate-800"><p className="text-[7px] font-black text-blue-500 uppercase mb-1">{c.stage.replace(/_/g, ' ')}</p><p className="text-[10px] text-slate-400 font-medium italic">"{c.comment_text}"</p></div>))}</div>
            </div>
          </div>
