...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-[#fbfbfa] dark:bg-slate-950 text-[#09090b] dark:text-white font-sans selection:bg-indigo-500/30 overflow-x-hidden transition-colors duration-500">
      ...
      <div className={`fixed inset-0 z-[100] bg-[#fbfbfa]/95 dark:bg-slate-950/95 backdrop-blur-xl transition-transform duration-700 ease-in-out ${menuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="p-8 h-full flex flex-col">
          ...
          <div className="flex-1 flex flex-col justify-center space-y-12">
            ...
            <div className="grid md:grid-cols-2 gap-12 pt-12 border-t border-black/5 dark:border-white/5">
              <div className="space-y-4">
                <p className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-400 dark:text-slate-500">Connect</p>
              ...
              <div className="space-y-2">
                <p className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-400 dark:text-slate-500">Protocol</p>
                <p className="text-xs font-medium text-slate-500 italic uppercase tracking-widest">Trust, at Infrastructure Scale.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-20 items-center">
          <div className="relative z-10 space-y-10 animate-in fade-in slide-in-from-left-8 duration-1000">
            ...
            <div className="flex flex-col sm:flex-row gap-6">
              <Link href="/book" className="group bg-indigo-600 text-white px-10 py-5 rounded-2xl text-[11px] font-black uppercase tracking-[0.2em] hover:bg-indigo-700 transition-all flex items-center justify-center gap-3 shadow-2xl shadow-indigo-600/20">
                Initiate Service <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
              </Link>
              <Link href="/track" className="bg-white dark:bg-slate-900 border border-black/5 dark:border-white/10 px-10 py-5 rounded-2xl text-[11px] font-black uppercase tracking-[0.2em] hover:bg-[#fbfbfa] dark:hover:bg-slate-800 transition-all flex items-center justify-center text-[#09090b] dark:text-white shadow-sm">
                Active Tracking
              </Link>
            </div>
          </div>
          <div className="relative hidden lg:block animate-in fade-in slide-in-from-right-8 duration-1000">
            ...
            <div className="relative border border-black/5 dark:border-white/5 bg-white/50 dark:bg-white/5 backdrop-blur-2xl rounded-[3rem] p-4 shadow-2xl overflow-hidden group">
               <div className="absolute inset-0 bg-gradient-to-br from-indigo-600/20 via-transparent to-purple-600/20 opacity-0 group-hover:opacity-100 transition-opacity duration-1000" />
               <img src="https://images.unsplash.com/photo-1593642632823-8f785ba67e45?auto=format&fit=crop&q=80&w=1200" alt="Tech" className="rounded-[2.2rem] grayscale group-hover:grayscale-0 transition-all duration-1000 scale-105 group-hover:scale-100" />
            </div>
          </div>
        </div>
      </section>
      ...
      {/* Trust Quote Section */}
      <section className="px-6 md:px-12 py-40 text-center relative overflow-hidden bg-[#fbfbfa] dark:bg-slate-950">
        <div className="max-w-4xl mx-auto relative z-10 space-y-8">
          <Globe className="mx-auto text-indigo-600 dark:text-indigo-400" size={40} />
          <h2 className="text-4xl md:text-7xl font-black uppercase tracking-tighter leading-[0.9] text-[#09090b] dark:text-white">
            Your device data is <span className="text-indigo-600 italic">yours.</span> We never sell or share it.
          </h2>
          <p className="text-[10px] font-black uppercase tracking-[0.5em] text-slate-400 dark:text-slate-500 italic">Cepheus Technology Protocol · Infrastructure Scale Trust</p>
        </div>
      </section>

      {/* Professional Footer - FIXED CONTRAST */}
      <footer className="px-6 md:px-12 py-32 bg-white dark:bg-slate-900 border-t border-black/5 dark:border-white/5 transition-colors">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-20">
            ...
            <FooterColumn title="Framework" links={['Privacy Policy', 'Terms of Service', 'Audit Guidelines']} />
          </div>
          ...
          <div className="mt-32 pt-12 border-t border-black/5 dark:border-white/5 flex flex-col md:flex-row justify-between items-center gap-8">
            ...
            <div className="flex gap-8">
              <Instagram className="text-slate-400 dark:text-slate-500 hover:text-indigo-600 transition-colors cursor-pointer" size={18} />
              <Linkedin className="text-slate-400 dark:text-slate-500 hover:text-indigo-600 transition-colors cursor-pointer" size={18} />
              <Mail className="text-slate-400 dark:text-slate-500 hover:text-indigo-600 transition-colors cursor-pointer" size={18} />
            </div>
          </div>
        </div>
      </footer>
...
function FeatureCard({ icon, title, desc }) {
  return (
    <div className="group p-10 rounded-[3rem] bg-[#fbfbfa] dark:bg-slate-950 border border-black/5 dark:border-white/5 hover:border-indigo-600/30 hover:shadow-2xl transition-all duration-700">
      <div className="w-14 h-14 rounded-2xl bg-indigo-600/10 flex items-center justify-center text-indigo-600 dark:text-indigo-400 mb-8 group-hover:scale-110 transition-transform duration-700">
        {icon}
      </div>
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col space-y-4">
        {links.map((link) => (
          <Link
            key={link}
            href={link.toLowerCase().includes('book') ? '/book' : link.toLowerCase().includes('track') ? '/track' : `/${link.toLowerCase().replace(' ', '-')}`}
