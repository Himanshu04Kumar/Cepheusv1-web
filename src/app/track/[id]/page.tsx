...
export default function AdvancedTrackingPage() {
  ...
  const stages = [
    { id: 'BOOKED', title: 'Booking Confirmed', icon: CheckCircle2, desc: 'Slot allocated securely under transaction hash.' },
                  ...
                  >
                    ...
                    {!isFuture && (
                      <div className={`transition-transform duration-300 ${isExpanded ? 'rotate-180' : ''}`}>
                        <ChevronDown size={18} className={`${isCompleted ? 'text-green-600' : isActive ? 'text-amber-500' : 'text-slate-600'}`} />
                      </div>
                    )}
                  </button>
                                 ...
                                 <div key={opt.id} className="bg-slate-950 border border-slate-800 p-5 rounded-2xl flex justify-between items-center group hover:border-amber-500/50 transition-all shadow-xl">
                                    ...
                                    <div className="text-right">
                                       ...
                                       {booking.status === 'AWAITING_APPROVAL' && (
                                         <button onClick={(e) => { e.stopPropagation(); handleApprove(opt); }} className="mt-2 bg-amber-600 text-[9px] font-black uppercase tracking-widest px-4 py-2 rounded-xl text-white hover:bg-amber-500 shadow-lg shadow-amber-600/20 transition-all">Approve & Pay</button>
                                       )}
                                    </div>
                                 </div>
                              ))}
