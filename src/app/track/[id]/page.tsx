    ...
    <div className="min-h-screen bg-slate-950 text-white p-4 md:p-8 transition-colors selection:bg-blue-500/30">
      <div className="max-w-3xl mx-auto space-y-12">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-slate-900 pb-8">
          <h1 className="text-4xl font-black uppercase tracking-tighter text-blue-500 italic">Registry Status</h1>
        ...
        <div className="space-y-4 relative before:absolute before:left-[11px] before:top-4 before:bottom-4 before:w-0.5 before:bg-slate-900">
            ...
            return (
              <div key={stage.id} className={`relative pl-12 transition-all duration-500 ${isFuture ? 'opacity-30' : 'opacity-100'}`}>
                ...
                <div className={`bg-slate-900/40 border transition-all duration-500 rounded-3xl overflow-hidden ${
                  isExpanded ? (isActive ? 'border-amber-500/50 bg-slate-900/80 shadow-2xl' : 'border-green-500/30 bg-slate-900/80 shadow-2xl') : 'border-slate-800'
                }`}>
                  <button
                  ...
                  >
                    <div>
                      <h3 className={`text-xs font-black uppercase tracking-widest transition-colors ${
                        isCompleted ? 'text-green-500' : isActive ? 'text-amber-500' : 'text-slate-600'
                      }`}>
                        {stage.title}
                      </h3>
                      <p className="text-[9px] text-slate-500 font-medium mt-1 uppercase tracking-tighter">{stage.desc}</p>
                    </div>
                    {!isFuture && (
                      <div className={`transition-transform duration-300 ${isExpanded ? 'rotate-180' : ''}`}>
                        <ChevronDown size={18} className="text-slate-600 group-hover:text-blue-500" />
                      </div>
                    )}
                  </button>
                    ...
                    <div className="px-6 pb-6 space-y-6 animate-in slide-in-from-top-4 duration-500">
                      ...
                      {stage.id === 'BOOKED' && (
                        <div className="grid grid-cols-2 gap-4 pt-2 border-t border-white/5">
                           <div><p className="text-[9px] font-black text-slate-600 uppercase mb-1">Hardware</p><p className="text-xs font-bold">{booking.device_brand} {booking.device_model}</p></div>
                           <div><p className="text-[9px] font-black text-slate-600 uppercase mb-1">Status</p><p className="text-xs font-bold text-green-500">₹99 Verified</p></div>
                        </div>
                      )}
                        ...
                        <div className="space-y-4 pt-4 border-t border-white/5">
                           <div className="space-y-3">
                              {options.filter(o => !o.option_name.includes('CALL US')).map((opt) => (
                                 <div key={opt.id} className="bg-slate-950 border border-slate-800 p-5 rounded-2xl flex justify-between items-center group hover:border-amber-500/50 transition-all shadow-xl">
                                    ...
                                    <div className="text-right">
                                       ...
                                       {booking.status === 'AWAITING_APPROVAL' && (
                                         <button onClick={(e) => { e.stopPropagation(); handleApprove(opt); }} className="mt-2 bg-blue-600 text-[9px] font-black uppercase tracking-widest px-4 py-2 rounded-xl text-white hover:bg-blue-500 transition-all">Approve & Pay</button>
                                       )}
                                    </div>
                                 </div>
                              ))}
                           </div>
                           <button onClick={(e) => { e.stopPropagation(); handleRequestCallback(); }} className="w-full bg-slate-900 border border-blue-500/30 text-blue-400 py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-blue-600 hover:text-white transition-all shadow-lg">
